...
| `preveri-meja-pocitka.mjs` | meja najmanjšega počitka je ena sama številka na obeh straneh: `PRIVZETA_PRAVILA.minPocitekUr` (brskalnik, obrazec.html) in `public.min_pocitek()` (baza, `pocitek_ustreza`). Prej sta se razhajali (10,7 h proti 11 h), zato je bila menjava, ki jo je obrazec pokazal kot v redu, v bazi zavrnjena oz. sodelavec sploh ni bil ponujen | nič |
| `preveri-izvoz-excel.mjs` | `export-utils.js`/`export-buttons.js` po prehodu na ExcelJS (avgust 2026): `export-utils.js` je zdaj ZGRAJEN iz `export-utils.entry.js` (`node build-export.mjs`) s pravim `import ExcelJS from "exceljs"`; pisanje je asinhrono (`workbook.xlsx.writeBuffer()`); pri velikem mesečnem razporedu (6000 vrstic) se vrstice dodajajo po kosih, gumb med izvozom prikaže "Izvažam …" in je onemogočen, `requestAnimationFrame` pa med izvozom teče naprej (zaslon ne zamrzne). Zahteva zgrajen `export-utils.js` (`node build-export.mjs` pred zagonom) | `playwright` + brskalnik (glej opombo pod tabelo) |
...
| `preveri-zelje-postavitev.mjs` | združena stran **Želje** na PRAVEM izrisu (cela `zelje.html`, podtaknjena sta samo prijava in Supabase odjemalec): en sam izbirnik skupine (»Vse« + 6 oddelkov + FLEXI + NZV) namesto dveh ločenih zavihkov z dvema izbirnikoma, izbira skupine hkrati filtrira razpredelnico IN seznam zapisanih želja, privzeto je izbrana svoja skupina, vodja/administrator pade v NZV po VLOGI (ne po stari kodi enote), navodilo in zgodovina sprememb sta zložena, nobena kartica ni prazna in nič ne sili v vodoravno drsenje na telefonu. Preveri tudi **oba konca mostu v Generator**: gumb sestavi pravi naslov za izbrano skupino in mesec (NZV ima dva, FLEXI nobenega, ker se ne generira), `admin.html` s tem naslovom se res odpre na tem oddelku in mesecu, brez napotila pa ostane pri privzetem — in da navaden zaposleni gumba sploh ne vidi | `playwright` + brskalnik (glej opombo pod tabelo) |
| `preveri-generiraj-napotilo.mjs` | `preberiNapotiloIzNaslova` (admin.html) — razčlenjevanje naslova `admin.html?tab=kalup&oddelek=C1&mesec=2026-10`, prek katerega stran **Želje** napoti v Generator z že izbranim oddelkom in mesecem: naslov je zunanji vhod, zato se zavihek, oddelek (samo 6 kod rotacijskega generatorja — FLEXI in NZV se zavržeta), podzavihek NZV in oblika meseca preverijo, neveljavna vrednost pa pomeni »uporabi privzeto«, ne napake | nič |
| `preveri-nzv-rocno-in-vrzeli.mjs` | Mreža NZV: (1) `razberiCelico` — ročno prepisana celica se prebere nazaj v osebe (kratica »BOJ«, celo ime, več oseb, oznaki `?`/`*`), neprepoznano in dvoumno pa se **ne ugane**, ampak javi; (2) `predlagajZapolnitev` (»Predlagaj mesec«) — najde vrzeli, ne predlaga odsotnih, ne polni prostih dni, stolpcev odsotnosti, dežurstva ali tiste SA, ki ta dan ni na vrsti, in obremenitve ne nabere na eni osebi; (3) `rocniPopravkiVMrezi` — kdaj je popravek popravek in kaj gre iz njega v objavo; (4) `publishLeadScheduleRows` — druga enota iste osebe se ohrani v `pokriva_oddelek` in ne izgine | nič |
| `preveri-vrzeli-oddelek.mjs` | »Predlagaj mesec« za razpored oddelka (`Generator.predlagajZapolnitevOddelka`): predlaga tistega, ki tisti dan ne dela (prost dan pred KPU), **ne** osebe na dopustu, **ne** tistega, ki že dela, in **ne** predloga, ki bi ustvaril novo delovnopravno kršitev — če drugega ni, ga označi; ista oseba ne dobi dveh vrzeli istega dne, obremenitev se porazdeli med enako razpoložljivimi, izid je ponovljiv | nič |
| `preveri-nzv-nosilec-oddelka.mjs` | Kdo iz NZV je posamezen dan zadolžen za posamezen **oddelek** (`NzvZasedba.zasedbaEnot`): veriga nadomeščanja po uporabnikovem pravilu (Salkić C1 odsotna → C1 prevzame Arnež, Lunar je hkrati na B **in** C), velja tudi za en sam dan, brez nadomeščevalca oddelek ostane brez nosilca in se to vidi, vikendi/prazniki so prazni, izid pa se ujema z mrežo NZV (`razporedDneva`). Preverja tudi, da **objavljen razpored NZV prevlada nad pravilom** (ročni vpis koordinatorja se prenese na oddelek, dan brez objave pa se še naprej izpelje) in da `enoteIzZapisa` prebere vse enote iz `pokriva_oddelek`, dežurstva pa ne šteje za enoto | nič |
| `preveri-zavihki-razpored.mjs` | Prva stran Razporeda ima **štiri** zavihke (Moj razpored, Oddelki, Razpredelnica, Dežurstvo): na telefonu ikona + drobna beseda, na širokem zaslonu nespremenjeno besedilo; Razpredelnica je iz Imenika **prenesena** (tam je ni več); zavihek Dežurstvo pokaže tri kroge uradnega dokumenta, dežurno sestro pa vzame iz **objavljenega razporeda**, da so potrjene menjave upoštevane (uradni PDF le kot rezerva, označena z `*`). Preverja tudi, da **Razpredelnica pokaže cel mesec na en zaslon** — vseh 31 dni, brez vodoravnega vlečenja in brez stare omejitve 1400 px | Chromium (`CHROMIUM_PATH`) |
| `preveri-enote-in-navigacija.mjs` | Zapis enot (`normalizirajNazivOddelka`): **najprej lastna**, nato prevzeta, in vedno uradni naziv — »ŽO«, ne koda »ZO«, pri čemer se zamenjava naredi po **celi besedi** (sicer »IZOB« postane »IŽOB«); sestavljene enote (»UA/SA«, »B1,B2«) se ne razbijejo, ker v uradni predlogi pomenijo celoto; naziv iz šifranta (»B – oddelek«) ostane nedotaknjen. Preverja tudi vrstni red navigacije, da sta v Imeniku ostala samo Seznam in Parafe, in da je Stanje dopusta v Statistiki | Chromium (`CHROMIUM_PATH`) |
| `preveri-nzv-nosilec-brskalnik.mjs` | Isto na obeh straneh v pravem brskalniku: stolpec **NZV** na Razpored → Po oddelkih pokaže na dan dopusta nadomeščevalca in ne odsotne osebe, v Imenik → Razpredelnica pa so nosilci našteti pod oddelčnim kadrom — ista oseba se pojavi pod dvema oddelkoma, kadar pokriva oba, ročno objavljeni vpis pa pride do obeh zaslonov | Chromium (`CHROMIUM_PATH`) |
| `preveri-vrzeli-oddelek-brskalnik.mjs` | Ista pot v pravem brskalniku: gumb je na strani, vrzeli se izpišejo, **nič ne gre v razpored brez potrditve** (gumb »Vnesi potrjene« je onemogočen), potrjeni predlog pa res spremeni celico v mreži in izgine iz seznama | Chromium (`CHROMIUM_PATH`) |
| `preveri-maticna-stevilka.mjs` | Povezovanje oseb med viri: `Imena.kazalo` išče najprej po **matični številki**, ime pa je rezerva in se primerja prek `Imena.kljuc`, ne dobesedno (velike črke, izgubljene strešice, obrnjen vrstni red, dvobesedni priimek); podvojenega imena ne ugane, matična številka ga razreši. Preveri tudi, da uvoz imenika sprejme vrstico **brez e-pošte, a z matično številko**, da se glava »Mat.št« prepozna, in da objava razporeda ne uporablja več dobesednega `.in("full_name", …)` — prav to ni našlo nikogar | nič |
| `preveri-nzv-vrzeli-v-brskalniku.mjs` | Ista pot v pravem brskalniku: gumb »Predlagaj mesec« je na strani, najdena vrzel pride v mrežo in v seznam za potrditev, predlagana ni oseba na dopustu, predlog pa ostane **nepotrjen** (0/1) — objavi se šele, ko ga koordinator potrdi | Chromium (`CHROMIUM_PATH`) |
| `preveri-sheets-deljena-koda.mjs` | `src/shared/sheets-koordinate.js` (kopijo uporabljata robni funkciji za sinhronizacijo) se ni razšel z izvirniki: šifrant izmen se ujema z `izmene.js` vrstico za vrstico, tabeli popravkov imen z `imena.js`/`parafa.js`, `normalizirajDatum`/`najdiZamikStolpcev` z `import-utils.js`, koordinate celic pa dajo iste (vrstica, stolpec) kot `pripraviPosodobitveOddelka` v `index.html` — z zamikom stolpcev in brez njega. Preveri tudi, da je kopija v `supabase/functions/_shared/` bajt za bajt enaka | nič |
| `preveri-sheets-brez-postavitve.mjs` | Statično nad VSO kodo sinhronizacije: edina klica Google Sheets API sta `values.get` in `values:batchUpdate` — nikjer `values.append`, `insertDimension`, `mergeCells`, `repeatCell`, `addSheet` ipd. Koda torej ne more dodati ali izbrisati vrstice, stolpca ali zavihka niti pomotoma; Apps Script na dokumentu samo bere in pošlje koordinato, v preglednico ne piše. Namen: da tak klic pade v preizkusu, ne na uradnem podpisanem dokumentu | nič |
| `preveri-sheets-sinhronizacija.mjs` | `supabase/sheets-sinhronizacija.sql` na pravi bazi: izhodna vrsta se polni SAMO za povezave, ki so aktivne in imajo vklopljeno smer; ista celica čaka kvečjemu enkrat in v njej je zadnje stanje; sprememba brez razlike ne sproži ničesar; sprememba z `razlog='sheets'` se ne vrača v Sheets (zaščita pred neskončno zanko); izbris se pošlje kot prazna vrednost; `sheet_sync_prevzemi` istih vrstic ne vzame dvakrat in padle poskuša znova z naraščajočim zamikom, po petih poskusih pa jih pusti vidne; isto ime zavihka v DRUGEM dokumentu vodi na svoj oddelek | lokalni PostgreSQL + `su postgres` |
