...
--
-- =====================================================================
-- POZOR – VRSTNI RED ZAGONA NA OBSTOJEČI BAZI
--
-- Če baza že vsebuje podatke pod STARIMI (angleškimi) imeni tabel
-- (profiles, departments, schedule_entries …), poženi NAJPREJ
--   supabase/00-preimenuj-tabele.sql
-- in šele nato to datoteko. Brez tega bi spodnji "create table if not
-- exists" ustvaril 25 PRAZNIH novih tabel poleg starih - aplikacija bi
-- kazala prazen razpored, podatki pa bi tiho obtičali v starih tabelah.
-- Na povsem novi bazi tega koraka ni treba.
--
-- VRSTNI RED ZNOTRAJ TE DATOTEKE (odvisnosti gredo od zgoraj navzdol,
-- zato datoteka teče od začetka do konca brez napak manjkajočih
-- odvisnosti - preverjeno na PostgreSQL 16, 0 napak):
--
--   1. Razširitve .......................... razdelek 0
--   2. Glavne tabele (brez zapletenih FK) .. razdelka 1 (oddelki), 2 (profili)
--   3. Relacijske/transakcijske tabele ..... razdelki 3 (razpored),
--      4 (zahtevki_za_menjavo), 5 (obvestila), 6b (imenik), 9 (odsotnosti),
--      10 (nosilci_oddelkov), 12–19, 26–35
--   4. Pomožne funkcije za RLS ............. razdelek 6
--   5. RLS politike ........................ razdelek 7 (+ pri vsaki
--      poznejši tabeli takoj za njeno definicijo)
--   6. RPC funkcije in sprožilci ........... razdelka 8, 19b in naprej
--   7. Pogledi ............................. razdelki 6b, 11, 19, 33
--
-- Opomba: tabele, dodane po prvotni shemi (razdelki 9+), imajo svoje
-- politike/indekse/sprožilce namenoma ZRAVEN definicije tabele, ne v
-- skupnem bloku - tako je vsak razdelek samostojen in ga je mogoče brati
-- (in po potrebi pognati) kot celoto.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- 1) oddelki – 6 oddelkov SMS/TZN + dežurni/nedežurni kader
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2) profili – 1:1 z auth.users, nosi vlogo in oddelek/ekipo
-- ---------------------------------------------------------------------
create table if not exists public.profili (
  id uuid primary key references auth.users (id) on delete cascade,
  full_name text not null,
  role text not null default 'user' check (role in ('admin', 'vodja', 'user')),
  department_code text references public.oddelki (code) on update cascade,
  created_at timestamptz not null default now()
);

-- Varovalka: "create table if not exists" zgoraj je no-op, če "profili"
...
alter table public.profili add column if not exists role text not null default 'user';
alter table public.profili add column if not exists department_code text;
alter table public.profili add column if not exists created_at timestamptz not null default now();

alter table public.profili drop constraint if exists profiles_role_check;
alter table public.profili add constraint profiles_role_check check (role in ('admin', 'vodja', 'user'));
...
-- pravic ne more zapisati v public.profili.
--
...
grant all on public.profili to supabase_auth_admin;
...
-- ---------------------------------------------------------------------
-- 3) razpored – en zapis = en zaposleni, en dan, ena izmena
-- ---------------------------------------------------------------------
create table if not exists public.razpored (
  id bigint generated always as identity primary key,
  employee_id uuid not null references public.profili (id) on delete cascade,
  department_code text not null references public.oddelki (code) on update cascade,
  work_date date not null,
  shift_code text not null default '',
  updated_at timestamptz not null default now(),
  unique (employee_id, work_date)
);
create index if not exists schedule_entries_date_idx on public.razpored (work_date);
create index if not exists schedule_entries_dept_idx on public.razpored (department_code, work_date);

-- ---------------------------------------------------------------------
-- 4) zahtevki_za_menjavo – dvostopenjska odobritev (vodja → admin)
-- ---------------------------------------------------------------------
create table if not exists public.zahtevki_za_menjavo (
  id bigint generated always as identity primary key,
  requester_id uuid not null references public.profili (id),
  requester_date date not null,
  target_id uuid not null references public.profili (id),
  ...
  lead_id uuid references public.profili (id),
  lead_decided_at timestamptz,
  lead_note text,
  admin_id uuid references public.profili (id),
...
create index if not exists swap_requests_requester_idx on public.zahtevki_za_menjavo (requester_id);
create index if not exists swap_requests_target_idx on public.zahtevki_za_menjavo (target_id);
create index if not exists swap_requests_status_idx on public.zahtevki_za_menjavo (status);

-- ---------------------------------------------------------------------
-- 5) obvestila – obveščanje samo znotraj aplikacije
-- ---------------------------------------------------------------------
create table if not exists public.obvestila (
  id bigint generated always as identity primary key,
  user_id uuid not null references public.profili (id) on delete cascade,
  swap_request_id bigint references public.zahtevki_za_menjavo (id) on delete cascade,
  message text not null,
  read_at timestamptz,
  created_at timestamptz not null default now()
);
create index if not exists notifications_user_idx on public.obvestila (user_id, read_at);
...
create trigger on_swap_status_change
  after update on public.zahtevki_za_menjavo
...
-- ---------------------------------------------------------------------
-- 6b) Imenik (kontakti) – e-pošta na profili, telefon v LOČENI tabeli
--     (telefoni_kontaktov), da nivojsko vidljivost zagotavlja navadna
--     vrstična RLS, ne krhko stolpčno omejevanje na že široko
--     poizvedovani tabeli "profili" (ki ima "select using (true)").
-- ---------------------------------------------------------------------
...
-- profili in torej vidna vsem). To je prava vrstična RLS: če vrstica ni
-- vidna klicatelju, PostgREST pri vgnezdenem povpraševanju
-- profili→telefoni_kontaktov vrne phone: null, ne napake.
drop policy if exists contact_phones_select on public.telefoni_kontaktov;
create policy contact_phones_select on public.telefoni_kontaktov
...
create policy contact_phones_upsert_own on public.telefoni_kontaktov
  for insert to authenticated with check (profile_id = auth.uid());
drop policy if exists contact_phones_update_own on public.telefoni_kontaktov;
create policy contact_phones_update_own on public.telefoni_kontaktov
  for update to authenticated using (profile_id = auth.uid()) with check (profile_id = auth.uid());

drop policy if exists contact_phones_admin_all on public.telefoni_kontaktov;
create policy contact_phones_admin_all on public.telefoni_kontaktov
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- uvozi_kontaktov: admin vnaprej naloži seznam vseh zaposlenih (ime,
...
create table if not exists public.uvozi_kontaktov (
  ...
  department_code text references public.oddelki (code) on update cascade,
  linked_profile_id uuid references public.profili (id) on delete set null,
  created_at timestamptz not null default now()
);
...
-- spodaj na bazi, kjer je uvozi_kontaktov že obstajala pred to spremembo).
...
create policy contact_imports_admin on public.uvozi_kontaktov
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- uvozi_kontaktov_javno: na izrecno željo morajo biti VSI vneseni podatki
-- takoj vidni v Imeniku, TUDI za še ne povezane (neregistrirane) osebe – ne
-- samo adminu v "Uvoz zaposlenih". Osnovna tabela uvozi_kontaktov ostaja
...
-- ne glede na RLS na uvozi_kontaktov – vidnost posameznih stolpcev namesto
-- tega vsili spodnja CASE logika, ovrednotena za VSAKEGA klicatelja posebej
-- (current_role_is bere iz profili glede na auth.uid() klicatelja).
...
grant select on public.uvozi_kontaktov_javno to authenticated;

-- kadrovski_podatki: dodatni HR podatki (šifra zaposlenega, datum rojstva,
-- naziv delovnega mesta, vodja, starševsko varstvo, letni dopust), ki se
-- prekopirajo iz uvozi_kontaktov ob "Poveži". Bolj občutljivi kot telefon
-- (rojstni datum!), zato ožja vidljivost kot pri telefoni_kontaktov: lastnik vidi
...
-- Ista past kot pri uvozi_kontaktov zgoraj (glej opombo tam) - leave_balance_*
...
create policy profile_hr_details_select on public.kadrovski_podatki
  for select to authenticated using (
...
create policy profile_hr_details_admin_write on public.kadrovski_podatki
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));
...
create policy departments_select on public.oddelki
  for select to authenticated using (true);

drop policy if exists departments_write_admin on public.oddelki;
create policy departments_write_admin on public.oddelki
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- profili: vsi prijavljeni vidijo vsa imena (potrebno za izbiro sodelavca
-- pri predlogu menjave); ureja jih samo admin (vloga, oddelek, ime).
drop policy if exists profiles_select on public.profili;
create policy profiles_select on public.profili
  for select to authenticated using (true);

drop policy if exists profiles_update_admin on public.profili;
create policy profiles_update_admin on public.profili
  for update to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- pokriva_oddelek: en zaposleni lahko pokriva VEČ oddelkov (na izrecno
...
-- nosilci_oddelkov / obstoječega generatorja (admin.html) - ta še naprej
-- uporablja izključno profili.department_code (=primarni oddelek), da se
-- ne tvega regresij v že delujočem generiranju razporeda. Sprožilec spodaj
-- profili.department_code drži usklajen s "primarnim" vnosom tukaj, ne
-- glede na to, kateri del aplikacije department_code spremeni.
create table if not exists public.pokriva_oddelek (
  id uuid primary key default gen_random_uuid(),
  profile_id uuid not null references public.profili (id) on delete cascade,
  department_code text not null references public.oddelki (code) on update cascade,
...
create policy profile_departments_select on public.pokriva_oddelek
  for select to authenticated using (true);
drop policy if exists profile_departments_admin_write on public.pokriva_oddelek;
create policy profile_departments_admin_write on public.pokriva_oddelek
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));
...
create trigger trg_sync_primary_department
  after insert or update of department_code on public.profili
...
-- razpored: berejo vsi prijavljeni, za VSE oddelke (dogovorjeno);
-- pišejo (kalup/dežurstva) samo admin.
drop policy if exists schedule_select on public.razpored;
create policy schedule_select on public.razpored
  for select to authenticated using (true);

drop policy if exists schedule_write_admin on public.razpored;
create policy schedule_write_admin on public.razpored
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- zahtevki_za_menjavo: neposreden insert/update je zavrnjen (privzeto, ker ni
...
create policy swap_select on public.zahtevki_za_menjavo
  for select to authenticated using (
    ...
    or (
      ...
      and public.current_department() = (select department_code from public.profili where id = requester_id)
    )
  );

-- obvestila: vsak vidi in označi kot prebrano samo svoje.
drop policy if exists notifications_select on public.obvestila;
create policy notifications_select on public.obvestila
  for select to authenticated using (user_id = auth.uid());

drop policy if exists notifications_update_own on public.obvestila;
create policy notifications_update_own on public.obvestila
  for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());

-- ---------------------------------------------------------------------
-- 8) RPC funkcije – edina pot za pisanje v zahtevki_za_menjavo / izvedbo menjave
-- ---------------------------------------------------------------------
...
-- dejansko zamenja izmene v razpored (atomarno, en klic).
...
-- ---------------------------------------------------------------------
-- 9) odsotnosti – barvna razpredelnica dopustov/omejitev (zavihek
...
create policy leave_entries_select on public.odsotnosti
  for select to authenticated using (true);

-- Ime v odsotnosti.full_name je prosto besedilo iz statičnega
-- roster-seznama (npr. "BOJIĆ MATEJ"), profili.full_name pa karkoli je
...
create policy leave_entries_write on public.odsotnosti
  for all to authenticated
  using (
...
create policy leave_entries_log_select on public.dnevnik_odsotnosti
...
-- authenticated na dnevnik_odsotnosti.
...
create trigger on_leave_entry_change
  after insert or update or delete on public.odsotnosti
  for each row execute function public.log_leave_entry_change();

-- ---------------------------------------------------------------------
-- 10) nosilci_oddelkov – 22 vodij/nosilcev oddelkov (iz "Predloga
...
--     Ločeno od profili.department_code, ker gre za VODSTVENO
...
-- naprej brez sprememb (department_code ima tuji ključ na oddelki, ki
...
create policy lead_departments_select on public.nosilci_oddelkov
  for select to authenticated using (true);
drop policy if exists lead_departments_write_admin on public.nosilci_oddelkov;
create policy lead_departments_write_admin on public.nosilci_oddelkov
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));
...
--    kadrovski_podatki.employee_code, zato se prek njega samodejno
...
create table if not exists public.zgodovina_stanja_dopusta (
  ...
  profile_id uuid references public.profili (id) on delete set null,
...
comment on column public.zgodovina_stanja_dopusta.dnevi is 'Preostali dnevi dopusta na prvi dan meseca (stolpec DOPUST v Kadrisu)';

create index if not exists idx_leave_balance_history_obdobje on public.zgodovina_stanja_dopusta (leto, mesec);
create index if not exists idx_leave_balance_history_profile on public.zgodovina_stanja_dopusta (profile_id);
...
create policy leave_balance_history_select on public.zgodovina_stanja_dopusta
  for select to authenticated using (
...
create policy leave_balance_history_admin_write on public.zgodovina_stanja_dopusta
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));
...
create or replace view public.stanje_dopusta_pregled
...
create or replace view public.stanje_dopusta_obdobja
with (security_invoker = true) as
...
-- Ob vsakem uvozu drži kadrovski_podatki.leave_balance_days/asof usklajena
...
create trigger trg_sync_leave_balance
  after insert or update of dnevi, profile_id on public.zgodovina_stanja_dopusta
  for each row execute function public.sync_leave_balance_to_hr_details();

-- ---------------------------------------------------------------------
-- 12) barvne_oznake – pomni, katera barva celice v uvoženem barvnem
--     koledarju odsotnosti (Kadris) pomeni katero vrsto (odsotnosti.kind),
--     da administratorju vsak mesec ni treba znova ročno določati istih
--     barv. Sam uvoz piše neposredno v obstoječ odsotnosti (upsert, brez
...
comment on table public.barvne_oznake is 'ARGB barva celice iz uvoženega Excela -> vrsta odsotnosti (odsotnosti.kind); prezri=true pomeni "ni odsotnost, ne uvažaj"';
...
create policy absence_color_map_admin on public.barvne_oznake
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));

-- ---------------------------------------------------------------------
-- 13) dnevnik_ogledov – revizijska sled za "Prijavi se kot" (admin vidi
...
create table if not exists public.dnevnik_ogledov (
  ...
  admin_email text,
  target_profile_id uuid not null references public.profili (id) on delete cascade,
...
create index if not exists idx_admin_view_as_log_admin on public.dnevnik_ogledov (admin_id);
...
create policy admin_view_as_log_select on public.dnevnik_ogledov
  for select to authenticated using (public.current_role_is('admin'));
...
create policy admin_view_as_log_insert on public.dnevnik_ogledov
...
create policy admin_view_as_log_update on public.dnevnik_ogledov
...
-- ---------------------------------------------------------------------
-- 14) zelje_zaposlenih – "Seznam želja" (zelje.html), razdeljen po
...
--     tu NI vezan na splošno tabelo oddelki (ki jih vse vsebuje),
...
create index if not exists idx_employee_wishes_dept on public.zelje_zaposlenih (department_code);
...
create policy employee_wishes_select on public.zelje_zaposlenih
  for select to authenticated using (
...
create policy employee_wishes_insert on public.zelje_zaposlenih
...
create policy employee_wishes_update on public.zelje_zaposlenih
  for update to authenticated
  ...
  with check (
    ...
    or (
      ...
      and full_name = (select full_name from public.profili where id = auth.uid())
    )
  );

drop policy if exists employee_wishes_delete on public.zelje_zaposlenih;
create policy employee_wishes_delete on public.zelje_zaposlenih
  for delete to authenticated using (public.current_role_is('admin') or created_by = auth.uid());

-- ---------------------------------------------------------------------
-- 15) profili.rotation_slot – nadomešča trdo kodirano drugo kolono
--     (rotacijska črka A-E) v admin.html WARDS_META.staff. RLS ni nova:
--     rotation_slot je navaden profili stolpec, torej ga že pokrivata
...
-- ---------------------------------------------------------------------
alter table public.profili add column if not exists rotation_slot text check (rotation_slot in ('A','B','C','D','E'));
...
-- ---------------------------------------------------------------------
-- 16) kadrovski_podatki.duty_* – osebne nastavitve dežurnega kadra
...
--     dalje računata živo iz razpored (glej nalozizDezurniKader v
...
alter table public.kadrovski_podatki add column if not exists duty_day_off text check (duty_day_off in ('PO','TO','SR','ČE','PE','SO','NE'));
...
create policy dept_min_select on public.minimalna_zasedba
  for select to authenticated using (true);

drop policy if exists dept_min_write on public.minimalna_zasedba;
create policy dept_min_write on public.minimalna_zasedba
  for all to authenticated
  using (public.current_role_is('admin'))
  with check (public.current_role_is('admin'));
...
--     obstoječega zahtevki_za_menjavo (menjave.html) – ta obrazec je širši
...
--     delovni-cas.js), česar zahtevki_za_menjavo ne počne.
--
--     Prilagojeno iz zunanjega referenčnega gradiva (druga aplikacija,
--     shema profili/zaposleni_kadris) na obstoječo shemo profili/
--     oddelki/razpored – brez "koordinator" kot 4. vloge:
--     ta korak opravi kdorkoli ima role='admin' (enako kot povsod drugod
--     v aplikaciji, npr. 2. stopnja pri zahtevki_za_menjavo).
-- ---------------------------------------------------------------------

-- Neposredni vodja – ločeno od kadrovski_podatki.manager_name (ki je
...
alter table public.profili add column if not exists vodja_id uuid references public.profili (id) on delete set null;
...
create table if not exists public.obrazci (
  ...
  vlagatelj_id uuid not null references public.profili (id) on delete restrict,
  sodelavec_id uuid references public.profili (id) on delete restrict,
  vodja_id uuid references public.profili (id) on delete set null,
  koordinator_id uuid references public.profili (id) on delete set null,
...
create table if not exists public.obrazci_dnevnik (
  ...
  uporabnik_id uuid references public.profili (id) on delete set null,
  ime_ob_dejanju text,
...
-- razpored.shift_code med vlagateljem in sodelavcem – isti vzorec
-- kot decide_swap_admin (glej zgoraj), samo brez zaposleni_kadris/mat_st
-- posrednika, ker so profili.id že stabilen ključ.
...
--     razpored – enak vzorec kot obstoječi decide_swap_admin.
-- ---------------------------------------------------------------------
alter table public.profili add column if not exists is_koordinator boolean not null default false;

update public.profili set is_koordinator = true where lower(email) = 'denis.dzamastagic@pb-begunje.si';

alter table public.zahtevki_za_menjavo drop constraint if exists swap_requests_status_check;
alter table public.zahtevki_za_menjavo add constraint swap_requests_status_check check (
  status in (
...
begin
  ...
  select role into v_requester_role from public.profili where id = auth.uid();
  select role into v_target_role from public.profili where id = p_target_id;
...
--     ločenih menjave.html (zahtevki_za_menjavo, dvostopenjski vodja→admin) +
--     obrazec.html (obrazci, štiristopenjski sodelavec→vodja→koordinator).
--     menjave.html je ukinjena (glej commit) - zahtevki_za_menjavo ostane v bazi
...
alter table public.zelje_zaposlenih drop constraint if exists employee_wishes_department_code_check;
alter table public.zelje_zaposlenih add constraint employee_wishes_department_code_check
  check (department_code in ('B', 'C', 'C1', 'D', 'E1', 'E2', 'FLEXI', 'NZV'));

-- ---------------------------------------------------------------------
-- 24) profili.parafa – kratka 2-4 črkovna parafa (npr. "BOJ", "DŽA"),
--     kot jo uporablja uradna predloga "Letni dopusti in omejitve za NZV"
--     namesto polnega imena v celicah. NAMENOMA na profili (ne
--     kadrovski_podatki), ker mora biti vidna VSEM prijavljenim, ne
--     samo lastniku/adminu - kadrovski_podatki ima ožjo RLS vidljivost
--     (glej opombo pri tej tabeli), profili pa že ima "vsi vidijo"
...
--
--     a) profili.job_title - naziv delovnega mesta za podpis (npr.
--        "dipl. zn., Strokovni vodja V"), NAMENOMA na profili (ne
--        kadrovski_podatki.position_name, ki ima ožjo RLS vidljivost -
--        samo lastnik/admin) - vsi prijavljeni morajo videti podpis pod
--        katerimkoli razporedom, ne samo svojim. Admin ureja v Imeniku.
--     b) razpored.created_by/updated_by - kdo je zapis prvič
...
alter table public.razpored add column if not exists created_at timestamptz not null default now();
alter table public.razpored add column if not exists created_by uuid references public.profili (id);
alter table public.razpored add column if not exists updated_by uuid references public.profili (id);
...
-- ---------------------------------------------------------------------
-- 26) dnevnik_razporeda – revizijska sled (audit log): kdo, kdaj in
...
--     schedule_write_admin politike na razpored sami.
-- ---------------------------------------------------------------------
...
create index if not exists schedule_entries_log_date_idx on public.dnevnik_razporeda (work_date);
create index if not exists schedule_entries_log_emp_idx on public.dnevnik_razporeda (employee_id, work_date);
...
create policy schedule_entries_log_select_admin on public.dnevnik_razporeda
  for select to authenticated using (public.current_role_is('admin'));
...
--     obvestila tabela je EDINI vir resnice ("kaj je treba povedati"),
...
alter table public.obvestila add column if not exists push_sent_at timestamptz;
alter table public.obvestila add column if not exists kljuc text;

create unique index if not exists notifications_kljuc_idx on public.obvestila (kljuc) where kljuc is not null;
create index if not exists notifications_push_pending_idx on public.obvestila (created_at) where push_sent_at is null;
...
create index if not exists push_subscriptions_profile_idx on public.potisne_narocnine (profile_id);
...
create policy push_subscriptions_own on public.potisne_narocnine
  for all to authenticated
...
--      zahtevki_za_menjavo tabeli in se torej nikoli več ne sproži – obrazci
...
--      uspešni objavi (ne sprožilec na razpored: objava je na
...
-- ---------------------------------------------------------------------
-- 31) profili.parafa_pred_oktobrom_2026 – parafa je bila za del kadra
--     PRENOVLJENA z veljavnostjo od 1.10.2026 (uradna sprememba, ne napaka
--     v aplikaciji) - profili.parafa (sekcija 24) odslej hrani NOVO
...
--     ostane prazen in profili.parafa velja ne glede na datum (glej
...
-- ---------------------------------------------------------------------
-- 32) dezurni_zdravniki – kateri ZDRAVNIK je dežuren na posamezen dan (dva
...
--     profil/uporabnik. Ločeno od razpored (ta tabela je za
...
create policy duty_doctors_write on public.dezurni_zdravniki for all to authenticated
...
-- ---------------------------------------------------------------------
-- 34) razpored.pokriva_oddelek – kateri oddelek oseba TA DAN
...
--     oddelki in tuji ključ jih zavrne, zato jih je uvoz doslej
...
--
--     `profili.full_name` je edino ime, ki ga oseba sama vpiše (ob
...
--     (nosilci_oddelkov.full_name, odsotnosti.full_name,
--     uvozi_kontaktov.full_name, zgodovina_stanja_dopusta.full_name) so
...
--     profili.full_name, ki je zapisan izključno z velikimi črkami (torej
...
create trigger trg_standardiziraj_polno_ime
  before insert or update of full_name on public.profili
