-- ---------------------------------------------------------------------
-- Podvojena Lelič Dijana v lead_departments
--
...
--
-- Zakaj je do tega prišlo: lead_departments ima za primarni ključ IME,
...
delete from public.lead_departments l
 where translate(upper(l.full_name), 'ČŠŽĆĐ', 'CSZCD') like 'LELIC%'
   and coalesce(l.enote, '') = ''
   and exists (
     select 1 from public.lead_departments d
